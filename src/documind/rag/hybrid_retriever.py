...
from documind.models.search import Result
from documind.rag.indexer import COLLECTION_NAME, get_client
from documind.rag.retriever import search
...
def hybrid_search(
    ...
    all_chunks = [point.payload for point in points if point.payload is not None]
    ...
    return [Result(**chunk) for chunk in combined_search[:top_k]]
